...
define(function(require) {
	...
	createList.from = apSlice;
	createList.len = length;
	createList.reverse = apReverse;
	createList.head = head;
	createList.tail = tail;
	createList.initial = initial;
	createList.last = last;
	...
	function flattenDeep(list) {
		function flattenRecursive(result, x) {
			if (Array.isArray(x)) {
				result = apConcat(result, flattenDeep(x));
			} else {
				result.push(x);
			}
